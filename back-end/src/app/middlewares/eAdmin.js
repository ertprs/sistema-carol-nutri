...
module.exports = (req, res, next) => {
    try {
        ...
        // Verifica se existe duas partes no vetor. Caso seja diferente de 2, retorna um erro de token
        if(!parts.length === 2)
        ...
        jwt.verify(token, authConfig.secret, async (err, decoded) => {
            ...
            // Procura um usuario pelo id decodificado
            const user = await User.findById(decoded.params.id).select('+eAdmin')
            // Verifica se o usuario é administrador ou nao
